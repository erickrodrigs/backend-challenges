...
```rb
Animal.where(atype: 'Cachorro').average(:monthly_cost)
```

### Quantos cachorros existem no sistema?

```rb
Animal.where(atype: 'Cachorro').count
```

### Qual o nome dos donos dos cachorros (Array de nomes)

```rb
Person.joins(:animals).where(animals: { atype: 'Cachorro' }).map(&:name)
```

### Retorne as pessoas ordenando pelo custo que elas tem com os animais (Maior para menor)

```rb
Person.joins(:animals).group(:name).order(sum_monthly_cost: :desc).sum(:monthly_cost)
```

### Levando em consideração o custo mensal, qual será o custo de 3 meses de cada pessoa?

```rb
Person.joins(:animals).group(:name).order(sum_monthly_cost: :desc).sum(:monthly_cost).map { |key , value| [key, value * 3] }.to_h
```
